...
public class DynamicBoneCopier : MonoBehaviour
{
    ...
    public class MeshCombinerEditor : Editor
    {
        ...
        public const string copyright = "MIT License (C)2025 Beckadam";
        private bool are_you_sure = false;
        public override void OnInspectorGUI()
        {
            DynamicBoneCopier copier = (DynamicBoneCopier)target;
            ...
            if (GUILayout.Button(are_you_sure ? "Are you sure?" : "Remove Bones from Target"))
            {
                if (are_you_sure)
                {
                    are_you_sure = false;
                    copier.RemoveTree(copier.m_target);
                }
                else
                {
                    are_you_sure = true;
    ...
    public void RemoveTree(Transform target)
    {
        if (target == null)
        {
            return;
        ...
        {
            Transform item = target.GetChild(i);
            if (item.childCount > 0)
            {
                RemoveTree(item);
            }
            foreach (Component comp in item.GetComponents(typeof(DynamicBone)))
            {
    ...
    public void CopyTree(Transform source, Transform target)
    {
        for (int i = 0; i < source.childCount; i++)
        {
            ...
            Component[] components = item.GetComponents(typeof(DynamicBone));
            foreach (DynamicBone db in components) {
                if (db != null)
                {
                    DynamicBone targetDb = targetItem.gameObject.AddComponent<DynamicBone>();
